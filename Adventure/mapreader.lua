...
function TiledMap_Load (filepath, tilesize, spritepath_removeold, spritepath_prefix)
    ...
    local tiletype, animations, layers = TiledMap_Parse(filepath)
    gMapLayers = layers
    for first_gid, path in pairs(tiletype) do
        ...
        local gid = first_gid
        local e = kTileSize
        for y = 0, floor(h / (kTileSize + spacing)) - 1 do
          for x = 0, floor(w / (kTileSize + spacing)) - 1 do
            local sprite = love.image.newImageData(kTileSize, kTileSize)
            sprite:paste(raw, 0, 0, margin + x * (e + spacing), margin + y * (e + spacing), e, e)
...
function TiledMap_GetMapW () 
  return gMapLayers.width 
end

function TiledMap_GetMapH () 
  return gMapLayers.height 
end
...
function TiledMap_GetMapWUsed ()
    ...
    for layerid, layer in pairs(gMapLayers) do 
      if (type(layer) == "table") then 
        for ty, row in pairs(layer) do
          if (type(row) == "table") then 
            for tx, t in pairs(row) do 
...
function TiledMap_GetLayerZByName (layername) 
  for z, layer in ipairs(gMapLayers) do 
...
function TiledMap_SetLayerInvisByName (layername) 
  gTileMap_LayerInvisByName[layername] = true 
end
...
function TiledMap_DrawNearCam (camx, camy, fun_layercallback)
    ...
    for z = 1, #gMapLayers do 
      if (fun_layercallback) 
        then fun_layercallback(z, gMapLayers[z]) 
      end
...
function LoadXML(s)
  ...
  while true do
    ...
    --[[
      ni: Start of <XML> tag within the full file string.
      j : End of <XML> tag within the full file string.
      
      c : if '/' then notes the end of a set.
        ie.   <Map>     c: = 
              </Map>    c: = /
    
      label: the label of the current innermost set.
        ie.   <Map>     label = 'Map'
                <Frame> label = 'Frame'
                
      xarg:  The misc. arguments after the label
        ie.   <Map name="forest" level="3">
            xarg = 'name="forest" level="3"'
            
      
      empty:  A single unit.  Does not have an open/close
        ie.  <Map>    empty = 
             </Map>   empty = 
             
             <image source = "map.png />    empty = /
...
local function getAnimations(node)
  local currentLayer = 1
  local oldTileId = 0
  local animations = {}
  
  for k, sub in ipairs(node) do
    if (sub.label == "frame") then
      --print(sub.xarg.tileid)
    end
...
function TiledMap_Parse(filename)
    ...
    local animations = getAnimations(xml[2])
    local layers = getLayers(xml[2])
    return tiles, animations, layers
end